...
def divergence_bf(dx, y, **unused_kwargs):
    ...
    return sum_diag.contiguous() #d dx / dy
...
def divergence_approx(f, y, e=None): #eT*df/dy*e
    e_dzdx = torch.autograd.grad(f, y, e, create_graph=True)[0]
    e_dzdx_e = e_dzdx * e
    approx_tr_dzdx = e_dzdx_e.view(y.shape[0], -1).sum(dim=1)
    return approx_tr_dzdx  #eT*df/dy*e https://blog.csdn.net/Li7819559/article/details/104738893/


def sample_rademacher_like(y): #产生-1，1
    return torch.randint(low=0, high=2, size=y.shape).to(y) * 2 - 1 #0,1再*2-1


def sample_gaussian_like(y): #y同样维度的高斯分布
...
class ODEnet(nn.Module): #
    #网络 args.dims = '-'.join([str(args.hdim_factor * data.n_dims（列数）)] * args.nhidden)
    # '-'.join([str(hdim_factor=10 * data.n_dims=128)] * nhidden=2)
    """
    ...
    #hidden_dims=tuple(map(int, args.dims.split("-"))),args.dims='-'.join([str(args.hdim_factor * data.n_dims)] * args.nhidden)='2560'
    #input_shape=data.n_dims=256
        ...
        if conv:
            ...
            base_layer = {
                ...
                "concatsquash": diffeq_layers.ConcatSquashLinear, #concatsquash用这个layer
                ##x linear(in->out) * sigmoid(t linear(1->out)) + t linear(1->out)
        ...
        for dim_out, stride in zip(hidden_dims + (input_shape[0],), strides): #dim_out=[2560,256], strides=[None,None]
            ...
            layer = base_layer(hidden_shape[0], dim_out, **layer_kwargs) #base_layer(列, 2560， **layer_kwargs)
            layers.append(layer)  #append
            #1.base_layer(256, 2560, **layer_kwargs=None)
            #2.base_layer(2560, 256, **layer_kwargs=None)
    ...
    def forward(self, t, y):
        ...
        for _ in range(self.num_squeeze):
            dx = unsqueeze(dx, 2)
        return dx #y和t经过两层网络，和2个activation softplus
...
class ODEfunc(nn.Module):
    ...
    def __init__(self, diffeq, divergence_fn="approximate", residual=False, rademacher=False): #
        ...
        self.diffeq = diffeq #ODEnet
        self.residual = residual #false
        self.rademacher = rademacher #false

        if divergence_fn == "brute_force":
            self.divergence_fn = divergence_bf
        elif divergence_fn == "approximate":
            self.divergence_fn = divergence_approx #eT * df/dy * e

        self.register_buffer("_num_evals", torch.tensor(0.))

    def before_odeint(self, e=None):
        self._e = e
        self._num_evals.fill_(0) #迭代次数，初始0

    def num_evals(self): #返回迭代次数
    ...
    def forward(self, t, states):
        ...
        if self._e is None:
            if self.rademacher:
                ...
                self._e = sample_gaussian_like(y) #_e是高斯分布 
        ...
        with torch.set_grad_enabled(True):
            ...
            dy = self.diffeq(t, y, *states[2:])  #t和y经过ODEnet，2层linear,得到dy（256维）
            #x linear(in->out) * sigmoid(t linear(1->out)) + t linear(1->out),256->2560,2569->256
            # Hack for 2D data to use brute force divergence computation.
            if not self.training and dy.view(dy.shape[0], -1).shape[1] == 2:
                divergence = divergence_bf(dy, y).view(batchsize, 1) #d(dy)/ d(y)
            else:
                divergence = self.divergence_fn(dy, y, e=self._e).view(batchsize, 1)  #eT*d(dy)/d(y)*e
        if self.residual: #no
            dy = dy - y
            divergence -= torch.ones_like(divergence) * torch.tensor(np.prod(y.shape[1:]), dtype=torch.float32
                                                                     ).to(divergence)
        return tuple([dy, -divergence] + [torch.zeros_like(s_).requires_grad_(True) for s_ in states[2:]])
        #返回[#dy即t和y经过ODEnet4层linear的output，-d(dy)/ d(y)或者-eT*d(dy)/d(y)*e]
        #即[f即t和y经过ODEnet4层linear的output，-d(f)/ d(y)或者-eT*d(f)/d(y)*e],
        #not self.training and dy.view(dy.shape[0], -1).shape[1] == 2前者，else后者
