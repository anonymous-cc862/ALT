...
def plt_flow(prior_logdensity, transform, ax, npts=100, title="$q(x)$", device="cpu"): #画px
    #plt_flow(log高斯分布,model(z)->x)#model(z, reverse=True) #是model(z)得到x
    """
    Args:
        transform: computes z_k and log(q_k) given z_0
        给出z得到x和logpx,但z不一定是标准正态分布
    """
    side = np.linspace(LOW, HIGH, npts) #-4～4之间100个数
    xx, yy = np.meshgrid(side, side) #side重复100遍，side一个数重复100遍
    z = np.hstack([xx.reshape(-1, 1), yy.reshape(-1, 1)]) #[a,b],for b遍历：for a遍历

    z = torch.tensor(z, requires_grad=True).type(torch.float32).to(device)
    logqz = prior_logdensity(z) #对z log标准正态
    logqz = torch.sum(logqz, dim=1)[:, None]
    z, logqz = transform(z, logqz) #return model(z, logpz, reverse=True)得到x和logqx
    logqz = torch.sum(logqz, dim=1)[:, None]

    xx = z[:, 0].cpu().numpy().reshape(npts, npts)
    yy = z[:, 1].cpu().numpy().reshape(npts, npts)
    qz = np.exp(logqz.cpu().numpy()).reshape(npts, npts) #e(logpx)->px
...
def plt_flow_density(prior_logdensity, inverse_transform, ax, npts=100, memory=100, title="$q(x)$", device="cpu"):
    #plt_flow_density(standard_normal_logprob, model-1(x)->z）
    side = np.linspace(LOW, HIGH, npts)
    xx, yy = np.meshgrid(side, side)
    x = np.hstack([xx.reshape(-1, 1), yy.reshape(-1, 1)]) #此处是x，与上面z不同
    ...
    for ii in torch.split(inds, int(memory**2)):
        z_, delta_logp_ = inverse_transform(x[ii], zeros[ii]) #z
        z.append(z_)
        delta_logp.append(delta_logp_)
    z = torch.cat(z, 0)
    delta_logp = torch.cat(delta_logp, 0)

    logpz = prior_logdensity(z).view(z.shape[0], -1).sum(1, keepdim=True)  # x先由inverse_transform换为z，再logp(z)
    logpx = logpz - delta_logp

    px = np.exp(logpx.cpu().numpy()).reshape(npts, npts) #px
...
def plt_flow_samples(prior_sample, transform, ax, npts=100, memory=100, title="$x ~ q(x)$", device="cpu"):
    z = prior_sample(npts * npts, 2).type(torch.float32).to(device) #高斯分布
    zk = []
    inds = torch.arange(0, z.shape[0]).to(torch.int64)
    for ii in torch.split(inds, int(memory**2)):
        zk.append(transform(z[ii])) #transform(z）->x
    zk = torch.cat(zk, 0).cpu().numpy()
    ax.hist2d(zk[:, 0], zk[:, 1], range=[[LOW, HIGH], [LOW, HIGH]], bins=npts) #画x
    ax.invert_yaxis()
    ax.get_xaxis().set_ticks([])
    ax.get_yaxis().set_ticks([])
    ax.set_title(title)
...
def visualize_transform(
    ...
    #potential_or_samples是N(m,std)
    #prior_sample是torch.randn
    #prior_density是standard_normal_logprob，log高斯分布
    #transform是model(z)->x, 
    #inverse_transform：model-1(x)->z
    plt.clf()
    ax = plt.subplot(1, 3, 1, aspect="equal")
    if samples:
        plt_samples(potential_or_samples, ax, npts=npts)  #画N(m,std)
    else:
        plt_potential_func(potential_or_samples, ax, npts=npts)

    ax = plt.subplot(1, 3, 2, aspect="equal")
    if inverse_transform is None:
        plt_flow(prior_density, transform, ax, npts=npts, device=device) #由z得到x和logpx，画出px
        #plt_flow(log高斯分布,model(z)->x)
    else:
        plt_flow_density(prior_density, inverse_transform, ax, npts=npts, memory=memory, device=device)
        #plt_flow_density(standard_normal_logprob, model-1(x)->z）#由x得到z，再得到logpz，画px
